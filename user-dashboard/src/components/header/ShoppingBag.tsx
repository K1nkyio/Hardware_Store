...
const ShoppingBag = ({
  ...
  const formattedSubtotal = formatProductPrice({ priceCents: subtotalCents, currency });
              ...
              {[...Array(3)].map((_, index) => (
                <div key={index} className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="h-3 w-1/3 bg-muted/30" />
                    <div className="h-4 w-2/3 bg-muted/30" />
                    <div className="h-3 w-1/4 bg-muted/30" />
                  </div>
                </div>
              ))}
            </div>
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          {item.stock <= 0 && item.backorderable && (
                            <p className="text-xs text-amber-600">Backorder — ships in {item.backorderEtaDays ?? 7} days</p>
                          )}
                        </div>
                        <p className="text-sm font-light text-foreground">{formatProductPrice(item)}</p>
                      </div>
                      <div className="flex items-center gap-2 mt-3">
                        <div className="flex items-center border border-border">
                          ...
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                            className="p-2 hover:bg-muted/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            aria-label="Increase quantity"
                            disabled={
                              item.quantity >=
                              (item.stock > 0 ? item.stock : item.backorderable ? backorderLimit : 0)
                            }
                          >
                            <Plus size={14} />
                          </button>
