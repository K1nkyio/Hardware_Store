...
const app = express();
function isPrivateDevHost(origin) {
    try {
...
app.use(cors({
    origin: (origin, cb) => {
        ...
        if (config.corsOrigins.includes(normalizedOrigin))
            return cb(null, true);
        if (config.allowLocalhostCors && isPrivateDevHost(normalizedOrigin))
            return cb(null, true);
...
app.use("/api/admin/auth/login", createRateLimit({
    keyPrefix: "admin-login",
    windowMs: 60_000,
    max: 5,
}));
app.use("/api/auth/admin/login", createRateLimit({
...
app.use("/api/analytics/events", createRateLimit({
    keyPrefix: "analytics",
    windowMs: 60_000,
    max: 120,
}));
app.use("/api/marketing/abandoned-cart", createRateLimit({
    keyPrefix: "marketing",
    windowMs: 60_000,
    max: 60,
}));
...
app.use(async (err, req, res, _next) => {
    ...
    const errorLogLine = `[${req.method} ${req.originalUrl}] ${statusCode} ${message}${err instanceof Error && err.stack ? `\n${err.stack}` : ""}`;
    try {
