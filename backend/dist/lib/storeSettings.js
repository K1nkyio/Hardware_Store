...
function normalizeSettings(input) {
    ...
    if (parsed.success)
        return parsed.data;
    const source = (input ?? {});
    const currentStore = (source.store ?? {});
    const currentShipping = (source.shippingRates ?? {});
    const currentPayment = (source.payment ?? {});
    const currentTax = (source.tax ?? {});
    const resolveShippingRate = (primaryKey, legacyKeys, fallback) => {
    ...
    const merged = {
        store: {
            storeName: typeof currentStore.storeName === "string" && currentStore.storeName.trim()
                ? currentStore.storeName
                : DEFAULT_STOREFRONT_SETTINGS.store.storeName,
            phone: typeof currentStore.phone === "string"
                ? currentStore.phone
                : DEFAULT_STOREFRONT_SETTINGS.store.phone,
            email: typeof currentStore.email === "string"
                ? currentStore.email
                : DEFAULT_STOREFRONT_SETTINGS.store.email,
            address: typeof currentStore.address === "string"
                ? currentStore.address
                : DEFAULT_STOREFRONT_SETTINGS.store.address,
        },
        shippingRates: {
            standardHomeDeliveryCents: resolveShippingRate("standardHomeDeliveryCents", ["metroManilaCents", "provincialLuzonCents"], DEFAULT_STOREFRONT_SETTINGS.shippingRates.standardHomeDeliveryCents),
            expressDeliveryCents: resolveShippingRate("expressDeliveryCents", ["visayasMindanaoCents"], DEFAULT_STOREFRONT_SETTINGS.shippingRates.expressDeliveryCents),
        },
        payment: {
        ...
        tax: {
            taxRatePercent: typeof currentTax.taxRatePercent === "number" &&
            ...
            tin: typeof currentTax.tin === "string"
                ? currentTax.tin
...
export async function ensureStorefrontSettingsRow() {
    ...
    await pool.query(`INSERT INTO app_settings (key, value)
     VALUES ($1, $2::jsonb)
     ON CONFLICT (key) DO NOTHING`, [SETTINGS_KEY, JSON.stringify(DEFAULT_STOREFRONT_SETTINGS)]);
}
export async function getStorefrontSettings() {
    await ensureStorefrontSettingsRow();
    const result = await pool.query(`SELECT value, updated_at::text
     FROM app_settings
     WHERE key = $1`, [SETTINGS_KEY]);
...
export async function saveStorefrontSettings(nextSettings) {
    const validated = StorefrontSettingsSchema.parse(nextSettings);
    const result = await pool.query(`INSERT INTO app_settings (key, value)
     VALUES ($1, $2::jsonb)
     ON CONFLICT (key)
     DO UPDATE SET value = EXCLUDED.value, updated_at = now()
     RETURNING updated_at::text`, [SETTINGS_KEY, JSON.stringify(validated)]);
    return {
        ...validated,
