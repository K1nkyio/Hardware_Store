...
export async function validatePromoCode(input) {
    ...
    const totalUsagePromise = pool.query("SELECT COUNT(*)::int AS total FROM promo_code_redemptions WHERE promo_code_id = $1", [promo.id]);
    const userUsagePromise = input.userId && promo.per_user_limit
        ? pool.query("SELECT COUNT(*)::int AS total FROM promo_code_redemptions WHERE promo_code_id = $1 AND user_id = $2", [promo.id, input.userId])
    ...
    let discountCents = promo.discount_type === "percent"
