...
const REFRESH_COOKIE_BY_DOMAIN = {
    customer: "hs_customer_refresh",
    admin: "hs_admin_refresh",
};
const COOKIE_PATH_BY_DOMAIN = {
    customer: "/api/auth",
    admin: "/api",
};
...
export function readRefreshCookie(req, domain) {
...
export function setRefreshCookie(res, domain, refreshToken) {
...
export function clearRefreshCookie(res, domain) {
...
function buildAccessToken(user, sessionId, mfaPending = false) {
    if (user.role === "customer") {
        return signToken({
            ...
            name: user.fullName,
            type: "access",
        }, config.jwtAccessSecret, config.jwtAccessTtlSeconds);
    }
    return signToken({
        sub: user.id,
        role: user.role,
...
function buildRefreshToken(user, sessionId, mfaPending = false) {
    return signToken({
        sub: user.id,
        sid: sessionId,
        ...
        mfa: mfaPending ? "pending" : "verified",
        type: "refresh",
    }, config.jwtRefreshSecret, config.jwtRefreshTtlSeconds);
}
function parseRefreshToken(token) {
    ...
    if (!sub || !sid || (type && type !== "refresh"))
        return null;
    const domain = decoded.domain === "admin" ? "admin" : "customer";
...
async function ensureEmailNotUsed(email) {
    const normalized = normalizeEmail(email);
    const { rowCount } = await pool.query(`SELECT 1 FROM users WHERE lower(email) = $1
     UNION ALL
...
async function createCustomerSession(user, context) {
    const sessionId = randomUUID();
    const refreshToken = buildRefreshToken(user, sessionId);
    const accessToken = buildAccessToken(user, sessionId);
    await pool.query(`INSERT INTO user_sessions (id, user_id, refresh_token_hash, device, ip, expires_at, last_used_at)
     VALUES ($1, $2, $3, $4, $5, now() + ($6::int * interval '1 second'), now())`, [sessionId, user.id, hashToken(refreshToken), context.device, context.ip, config.jwtRefreshTtlSeconds]);
    return {
        sessionId,
        accessToken,
        refreshToken,
...
async function issueUserToken(userId, type, ttlSeconds) {
    const token = randomBytes(32).toString("hex");
    await pool.query(`INSERT INTO user_auth_tokens (user_id, token_type, token_hash, expires_at)
     VALUES ($1, $2, $3, now() + ($4::int * interval '1 second'))`, [userId, type, hashToken(token), ttlSeconds]);
    return token;
}
async function consumeUserToken(token, type) {
    const { rows } = await pool.query(`UPDATE user_auth_tokens
     SET used_at = now()
     WHERE token_hash = $1
       AND token_type = $2
       AND used_at IS NULL
       AND expires_at > now()
     RETURNING user_id`, [hashToken(token), type]);
...
export async function registerCustomer(payload) {
    ...
    await pool.query(`INSERT INTO user_profiles (user_id, name, phone, address)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (user_id)
     DO UPDATE SET name = EXCLUDED.name, phone = EXCLUDED.phone, address = EXCLUDED.address`, [rows[0].id, fullName, payload.phone ?? null, payload.address ?? null]);
...
export async function loginCustomer(payload, context) {
    ...
    if (isLocked(user.lockedUntil))
        throw new Error("Account temporarily locked due to repeated failures");
...
export async function refreshCustomerSession(refreshToken) {
    const payload = parseRefreshToken(refreshToken);
    if (!payload || payload.domain !== "customer")
    ...
    await pool.query(`UPDATE user_sessions
     SET refresh_token_hash = $1,
         ...
         last_used_at = now()
     WHERE id = $3`, [hashToken(nextRefresh), config.jwtRefreshTtlSeconds, payload.sid]);
    return {
        user,
...
export async function logoutCustomer(userId, sessionId, allSessions = false) {
    if (allSessions) {
    ...
    await pool.query(`UPDATE user_sessions
     ...
     WHERE id = $1
       AND user_id = $2`, [sessionId, userId]);
}
export async function listCustomerSessions(userId, currentSessionId) {
    const { rows } = await pool.query(`SELECT id::text, device, ip, created_at::text, last_used_at::text, expires_at::text, revoked_at::text
     FROM user_sessions
     WHERE user_id = $1
       AND expires_at > now()
...
export async function revokeCustomerSession(userId, sessionId) {
    await pool.query(`UPDATE user_sessions
     SET revoked_at = COALESCE(revoked_at, now())
     WHERE id = $1
       AND user_id = $2`, [sessionId, userId]);
...
async function createAdminSession(user, context, mfaPending) {
    const sessionId = randomUUID();
    ...
    await pool.query(`INSERT INTO admin_sessions (id, admin_id, refresh_token_hash, ip, device, expires_at, last_used_at)
     VALUES ($1, $2, $3, $4, $5, now() + ($6::int * interval '1 second'), now())`, [sessionId, user.id, hashToken(refreshToken), context.ip, context.device, config.jwtRefreshTtlSeconds]);
    return {
        sessionId,
        accessToken,
        refreshToken,
...
function encryptedMfaSecret(secret) {
...
function decryptMfaSecret(encoded) {
...
function generateTotp(secretBase32, timestampMs = Date.now()) {
...
function verifyTotp(secretBase32, code) {
    ...
    for (const offset of [-30_000, 0, 30_000]) {
        if (generateTotp(secretBase32, Date.now() + offset) === normalized)
            return true;
    }
    return false;
}
...
export async function seedSuperAdmin(payload, context) {
    const existing = await pool.query("SELECT COUNT(*)::text AS count FROM admins");
    ...
    await pool.query(`INSERT INTO admin_roles (admin_id, role)
     ...
     ON CONFLICT (admin_id) DO UPDATE SET role = EXCLUDED.role, assigned_at = now()`, [user.id]);
...
export async function loginAdmin(payload, context) {
    ...
    const superAdminNeedsSetup = admin.role === "super_admin" && (!admin.mfaEnabled || admin.mfaResetRequired || !secret);
    ...
    if (admin.mfaEnabled && secret && !admin.mfaResetRequired) {
        ...
        if (!ok)
            throw new Error("MFA code or recovery code is required");
    }
...
export async function refreshAdminSession(refreshToken) {
    ...
    await pool.query(`UPDATE admin_sessions
     SET refresh_token_hash = $1,
         expires_at = now() + ($2::int * interval '1 second'),
         last_used_at = now()
     WHERE id = $3`, [hashToken(nextRefresh), config.jwtRefreshTtlSeconds, payload.sid]);
...
export async function logoutAdmin(adminId, sessionId, allSessions = false) {
    if (allSessions) {
    ...
    await pool.query(`UPDATE admin_sessions
     SET revoked_at = COALESCE(revoked_at, now())
     WHERE id = $1
       AND admin_id = $2`, [sessionId, adminId]);
}
export async function listAdminSessions(adminId, currentSessionId) {
    const { rows } = await pool.query(`SELECT id::text, device, ip, created_at::text, last_used_at::text, expires_at::text, revoked_at::text
     ...
     WHERE admin_id = $1
       AND expires_at > now()
     ORDER BY last_used_at DESC NULLS LAST, created_at DESC`, [adminId]);
    return {
        currentSessionId: currentSessionId ?? "",
        sessions: rows.map((row) => ({
            id: String(row.id),
...
export async function revokeAdminSession(adminId, sessionId) {
    await pool.query(`UPDATE admin_sessions
     SET revoked_at = COALESCE(revoked_at, now())
     WHERE id = $1
       AND admin_id = $2`, [sessionId, adminId]);
}
export async function setupAdminMfa(adminId, adminEmail) {
    ...
    await pool.query(`UPDATE admins
     ...
     WHERE id = $1`, [adminId, encryptedMfaSecret(secret), backupCodes.map(hashBackupCode)]);
...
export async function activateAdminMfa(adminId, sessionId, code, context) {
    const { rows } = await pool.query(`SELECT id, email, full_name, role, mfa_secret, mfa_enabled, mfa_reset_required, status
     FROM admins
     WHERE id = $1
     LIMIT 1`, [adminId]);
    if (rows.length === 0)
    ...
    const row = rows[0];
    if (!row.mfa_secret)
        throw new Error("MFA setup has not been initialized yet");
...
export async function regenerateAdminRecoveryCodes(payload) {
    const { rows } = await pool.query(`SELECT mfa_secret, mfa_recovery_codes
     FROM admins
     WHERE id = $1
     LIMIT 1`, [payload.adminId]);
    if (rows.length === 0)
        throw new Error("Admin account not found");
    const row = rows[0];
    if (!row.mfa_secret)
    ...
    const secret = decryptMfaSecret(String(row.mfa_secret));
    if (!secret)
        throw new Error("Unable to read MFA secret for this account");
    let authorized = false;
    const mfaCode = payload.mfaCode?.trim() ?? "";
    if (mfaCode && verifyTotp(secret, mfaCode))
        authorized = true;
    const storedCodes = Array.isArray(row.mfa_recovery_codes) ? row.mfa_recovery_codes : [];
    const recoveryCode = payload.recoveryCode?.trim().toUpperCase() ?? "";
...
export async function resetAdminMfa(targetAdminId) {
    await pool.query(`UPDATE admins
     SET mfa_enabled = false,
         mfa_secret = NULL,
         mfa_recovery_codes = '{}',
         mfa_reset_required = true
     WHERE id = $1`, [targetAdminId]);
    await pool.query("UPDATE admin_sessions SET revoked_at = COALESCE(revoked_at, now()) WHERE admin_id = $1", [targetAdminId]);
}
async function issueAdminToken(adminId, ttlSeconds) {
    const token = randomBytes(32).toString("hex");
    await pool.query(`INSERT INTO admin_auth_tokens (admin_id, token_type, token_hash, expires_at)
     VALUES ($1, 'password_reset', $2, now() + ($3::int * interval '1 second'))`, [adminId, hashToken(token), ttlSeconds]);
    return token;
}
async function consumeAdminToken(token) {
    const { rows } = await pool.query(`UPDATE admin_auth_tokens
     SET used_at = now()
     WHERE token_hash = $1
       AND token_type = 'password_reset'
       AND used_at IS NULL
       AND expires_at > now()
     RETURNING admin_id`, [hashToken(token)]);
