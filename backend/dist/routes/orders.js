...
ordersRouter.get("/", requireRole("viewer"), async (req, res, next) => {
    try {
      ...
      LIMIT $${params.length}`, params);
        const pipelineRows = await pool.query(`SELECT status, COUNT(*)::int AS count
       FROM orders
       GROUP BY status`);
        const pipeline = ORDER_STATUSES.map((entry) => ({
            status: entry,
            count: Number(pipelineRows.rows.find((row) => String(row.status) === entry)?.count ?? 0),
        }));
...
ordersRouter.get("/:id", requireRole("viewer"), async (req, res, next) => {
    try {
      ...
      WHERE id = $1`, [id]);
        ...
        const itemRows = await pool.query(`SELECT
        id::text,
        product_id::text,
        product_name,
        sku,
      ...
      ORDER BY id`, [id]);
        ...
        res.json({
            ...
            items: itemRows.rows.map((item) => ({
                ...
                backorderEtaDays: typeof item.backorder_eta_days === "number" ? item.backorder_eta_days : null,
            })),
        });
    }
    catch (err) {
...
ordersRouter.get("/:id/invoice", requireRole("viewer"), async (req, res, next) => {
    try {
      ...
      WHERE id = $1`, [id]);
        if (orderRows.rowCount === 0)
            return res.status(404).json({ message: "Order not found" });
        const itemRows = await pool.query(`SELECT product_name, sku, quantity, price_cents, currency
       FROM order_items
       WHERE order_id = $1
       ORDER BY id`, [id]);
        const order = orderRows.rows[0];
...
ordersRouter.patch("/:id", requireRole("manager"), async (req, res, next) => {
    try {
        ...
        const normalizedTags = body.tags?.map((tag) => tag.trim()).filter(Boolean).slice(0, 25) ?? null;
...
ordersRouter.post("/:id/mark-shipped", requireRole("manager"), async (req, res, next) => {
    try {
       ...
       WHERE id = $1
       RETURNING id::text, status, shipped_at::text, tracking_number, updated_at::text`, [id, body.trackingNumber?.trim() || null]);
        if (rows.length === 0)
            return res.status(404).json({ message: "Order not found" });
        await writeAuditLog({
            actorId: actor.id,
...
ordersRouter.post("/", async (req, res, next) => {
    ...
    try {
        ...
        const { rows } = await client.query(`SELECT id, name, sku, price_cents, currency, stock, backorderable, backorder_eta_days, reorder_threshold
       FROM products
       WHERE id = ANY($1::uuid[])
       FOR UPDATE`, [uniqueIds]);
        if (rows.length !== uniqueIds.length) {
            ...
            throw new HttpError(409, `Some products are unavailable: ${missingIds.map((id) => id.slice(0, 8)).join(", ")}`);
        }
        const productMap = new Map(rows.map((row) => [
            row.id,
            {
                ...
                backorderEtaDays: typeof row.backorder_eta_days === "number" ? row.backorder_eta_days : null,
                reorderThreshold: typeof row.reorder_threshold === "number" ? row.reorder_threshold : LOW_STOCK_THRESHOLD,
            },
        ]));
        ...
        for (const item of body.items) {
            ...
            if (product.currency !== currency) {
                throw new HttpError(400, "Mixed currencies are not supported in one checkout. Please separate your order by currency.");
            }
            ...
            await client.query(`INSERT INTO stock_movements (product_id, movement_type, quantity, previous_stock, next_stock, reason, performed_by, metadata)
         VALUES ($1, 'out', $2, $3, $4, $5, $6, $7)`, [
            ...
            if (remainingStock <= product.reorderThreshold) {
                const existingAlert = await client.query("SELECT id FROM inventory_alerts WHERE product_id = $1 AND resolved_at IS NULL LIMIT 1", [item.productId]);
                if (existingAlert.rowCount === 0) {
                    await client.query(`INSERT INTO inventory_alerts (product_id, stock_level, threshold)
             VALUES ($1, $2, $3)`, [item.productId, remainingStock, product.reorderThreshold]);
                }
            }
       ...
       RETURNING id, created_at::text`, [
        ...
        for (const item of orderItems) {
            await client.query(`INSERT INTO order_items (order_id, product_id, product_name, sku, quantity, price_cents, currency, backordered, backorder_eta_days)
