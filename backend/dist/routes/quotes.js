...
quotesRouter.post("/", async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO quote_requests (
...
quotesRouter.get("/mine", requireAuth({ domain: "customer" }), async (req, res, next) => {
    try {
        ...
        const emailRows = await pool.query("SELECT email FROM users WHERE id = $1", [actor.id]);
        if (emailRows.rowCount === 0)
      ...
      ORDER BY qr.created_at DESC`, [emailRows.rows[0].email]);
        res.json({
            items: rows.map((row) => ({
                id: row.id,
