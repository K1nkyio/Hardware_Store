...
inventoryRouter.get("/low-stock", requireRole("viewer"), async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT id::text, name, sku, stock, reorder_threshold
       FROM products
       WHERE stock <= COALESCE($1::int, reorder_threshold)
       ORDER BY stock ASC
       LIMIT $2`, [normalizedThreshold, normalizedLimit]);
        res.json({
...
inventoryRouter.get("/overview", requireRole("viewer"), async (_req, res, next) => {
    try {
        const productStatsPromise = pool.query(`SELECT
      ...
      FROM products`);
        const lowStockPromise = pool.query(`SELECT id::text, name, sku, stock, reorder_threshold, updated_at::text
       FROM products
       WHERE stock <= reorder_threshold
       ORDER BY stock ASC
       LIMIT 25`);
        const supplierStatsPromise = pool.query(`SELECT COUNT(*)::int AS total_suppliers,
              COUNT(*) FILTER (WHERE is_active = true)::int AS active_suppliers
       FROM suppliers`);
        const movementPromise = pool.query(`SELECT m.id::text, m.product_id::text, p.name AS product_name, p.sku,
...
inventoryRouter.get("/suppliers", requireRole("viewer"), async (_req, res, next) => {
    try {
        const { rows } = await pool.query(`SELECT id::text, name, email, phone, lead_time_days, is_active, created_at::text, updated_at::text
       FROM suppliers
       ORDER BY name ASC`);
...
inventoryRouter.get("/products/:id/suppliers", requireRole("viewer"), async (req, res, next) => {
    try {
       ...
       ORDER BY m.preferred DESC, s.name ASC`, [productId]);
        res.json(rows.map((row) => ({
            productId: row.product_id,
...
inventoryRouter.patch("/products/:id/settings", requireRole("manager"), async (req, res, next) => {
    ...
    try {
        ...
        if (body.supplier) {
            const supplierUpsert = await client.query(`INSERT INTO suppliers (name, email, phone, lead_time_days)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (name)
         DO UPDATE SET
         ...
         RETURNING id::text, name, email, phone, lead_time_days`, [
            ...
            const supplierId = supplierUpsert.rows[0].id;
            await client.query(`INSERT INTO product_supplier_map
        ...
        const updatedProduct = await client.query(`SELECT id::text, reorder_threshold, stock
       FROM products
...
inventoryRouter.get("/movements", requireRole("viewer"), async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT m.id::text, m.product_id::text, p.name AS product_name, p.sku,
              m.movement_type, m.quantity, m.previous_stock, m.next_stock,
...
inventoryRouter.post("/movements", requireRole("manager"), async (req, res, next) => {
    ...
    try {
        ...
        await client.query("BEGIN");
        const productRows = await client.query("SELECT id::text, name, stock, reorder_threshold FROM products WHERE id = $1 FOR UPDATE", [body.productId]);
        ...
        const signedQuantity = body.movementType === "in" ? rawQuantity : body.movementType === "out" ? -rawQuantity : body.quantity;
        ...
        const movementRows = await client.query(`INSERT INTO stock_movements
       ...
       RETURNING id::text, created_at::text`, [
        ...
        if (nextStock <= threshold) {
            const existingAlert = await client.query("SELECT id FROM inventory_alerts WHERE product_id = $1 AND resolved_at IS NULL LIMIT 1", [body.productId]);
            if (existingAlert.rowCount === 0) {
                await client.query(`INSERT INTO inventory_alerts (product_id, stock_level, threshold)
           VALUES ($1, $2, $3)`, [body.productId, nextStock, threshold]);
            }
        }
        else {
            await client.query("UPDATE inventory_alerts SET resolved_at = now() WHERE product_id = $1 AND resolved_at IS NULL", [body.productId]);
        }
        await writeAuditLog({
            actorId: actor.id,
            actorRole: actor.role,
