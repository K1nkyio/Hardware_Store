...
analyticsRouter.post("/events", async (req, res, next) => {
    try {
        ...
        await pool.query(`INSERT INTO analytics_events (event_name, session_id, user_email, payload)
       VALUES ($1,$2,$3,$4)`, [body.eventName, body.sessionId ?? null, body.userEmail ?? null, body.payload ?? null]);
        res.status(204).send();
    }
    catch (err) {
        next(err);
...
analyticsRouter.get("/dashboard-summary", requireRole("viewer"), async (req, res, next) => {
    try {
        ...
        const funnelRowsPromise = pool.query(`SELECT event_name, COUNT(*)::int AS total_events,
       ...
       GROUP BY event_name`, [
        ...
        const zeroResultSearchPromise = pool.query(`SELECT
       ...
       LIMIT 10`, params);
        const abandonedCartsPromise = pool.query(`SELECT
       ...
       ORDER BY subtotal_cents DESC`);
        const outOfStockLossPromise = pool.query(`SELECT
       ...
       ORDER BY lost_revenue_cents DESC`, params);
        ...
        const byEvent = new Map(funnelRows.rows.map((row) => [
            row.event_name,
            {
        ...
        const abandonedTotal = abandonedByCurrency.reduce((acc, row) => {
        ...
        const lostSalesTotal = lostSalesByCurrency.reduce((acc, row) => {
