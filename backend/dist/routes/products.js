...
function extractImageUrlsFromSpecs(specs) {
    if (!specs)
        return [];
    const candidate = specs.imageUrls;
...
productsRouter.patch("/bulk/update", requireRole("manager"), async (req, res, next) => {
    ...
    try {
        ...
        const { rows } = await client.query(`SELECT id, stock, price_cents, reorder_threshold
       FROM products
       WHERE id = ANY($1::uuid[])
       FOR UPDATE`, [body.ids]);
        if (rows.length === 0) {
        ...
        for (const row of rows) {
            ...
            const nextStock = typeof body.setStock === "number"
                ? body.setStock
                : currentStock + (body.stockDelta ?? 0);
            const nextPrice = typeof body.setPriceCents === "number"
                ? body.setPriceCents
         ...
         WHERE id = $1
         RETURNING id::text, stock`, [row.id, normalizedStock, normalizedPrice, body.setStatus ?? null]);
            if (updateResult.rowCount === 0)
                continue;
            affectedIds.push(updateResult.rows[0].id);
            if (normalizedStock <= reorderThreshold) {
                const existingAlert = await client.query("SELECT id FROM inventory_alerts WHERE product_id = $1 AND resolved_at IS NULL LIMIT 1", [row.id]);
                if (existingAlert.rowCount === 0) {
                    await client.query(`INSERT INTO inventory_alerts (product_id, stock_level, threshold)
             VALUES ($1, $2, $3)`, [row.id, normalizedStock, reorderThreshold]);
                }
            }
            else {
                await client.query("UPDATE inventory_alerts SET resolved_at = now() WHERE product_id = $1 AND resolved_at IS NULL", [row.id]);
            }
        }
        await writeAuditLog({
            actorId: actor.id,
...
productsRouter.get("/:id/reviews", async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT id, product_id, rating, title, body, author_name, verified, created_at::text
...
productsRouter.post("/:id/reviews", async (req, res, next) => {
    try {
        ...
        if (productCheck.rowCount === 0)
            return res.status(404).json({ message: "Not found" });
        const { rows } = await pool.query(`INSERT INTO product_reviews (product_id, rating, title, body, author_name, verified)
       VALUES ($1,$2,$3,$4,$5,$6)
...
function mapModerationQuestion(row) {
    return {
        ...mapQuestion(row),
        productName: row.product_name,
        productSku: row.product_sku ?? "",
...
productsRouter.get("/:id/questions", async (req, res, next) => {
    try {
        const productId = req.params.id;
        const { rows } = await pool.query(`SELECT id, product_id, question, answer, author_name, author_email, created_at::text, answered_at::text
...
productsRouter.post("/:id/questions", async (req, res, next) => {
    try {
        ...
        if (productCheck.rowCount === 0)
            return res.status(404).json({ message: "Not found" });
        const { rows } = await pool.query(`INSERT INTO product_questions (product_id, question, author_name, author_email)
       VALUES ($1,$2,$3,$4)
...
productsRouter.get("/moderation/queue", requireRole("viewer"), async (req, res, next) => {
    try {
        ...
        if (q) {
            params.push(`%${q}%`);
            const idx = params.length;
            whereParts.push(`(base.product_name ILIKE $${idx} OR base.product_sku ILIKE $${idx} OR base.author_name ILIKE $${idx} OR base.message ILIKE $${idx})`);
        }
        params.push(limit);
...
productsRouter.get("/moderation/history", requireRole("viewer"), async (req, res, next) => {
    try {
        ...
        if (q) {
            params.push(`%${q}%`);
            const idx = params.length;
            whereParts.push(`(m.action ILIKE $${idx} OR m.template_key ILIKE $${idx} OR m.note ILIKE $${idx} OR m.actor_role ILIKE $${idx})`);
        }
        params.push(limit);
        const whereSql = whereParts.length > 0 ? `WHERE ${whereParts.join(" AND ")}` : "";
        const { rows } = await pool.query(`SELECT
...
productsRouter.get("/moderation/reviews/list", requireRole("viewer"), async (_req, res, next) => {
    try {
        const { rows } = await pool.query(`SELECT r.id, r.product_id, r.rating, r.title, r.body, r.author_name, r.verified, r.created_at::text,
...
productsRouter.patch("/moderation/reviews/:reviewId", requireRole("manager"), async (req, res, next) => {
    try {
        ...
        const reviewState = await pool.query(`SELECT verified
       FROM product_reviews
       WHERE id = $1::uuid
       LIMIT 1`, [reviewId]);
        ...
        const existingModeration = await pool.query(`SELECT id
       ...
       LIMIT 1`, [reviewId]);
        ...
        const { rows } = await pool.query(`UPDATE product_reviews
       SET verified = $2
       WHERE id = $1
       RETURNING id, product_id, rating, title, body, author_name, verified, created_at::text,
                 (SELECT name FROM products WHERE products.id = product_reviews.product_id) AS product_name,
                 (SELECT sku FROM products WHERE products.id = product_reviews.product_id) AS product_sku`, [reviewId, body.verified]);
        if (rows.length === 0)
            return res.status(204).send();
        const parsed = ModerationReviewRowSchema.parse(rows[0]);
        await pool.query(`INSERT INTO moderation_actions (entity_type, entity_id, action, template_key, note, actor_role)
       VALUES ($1,$2::uuid,$3,$4,$5,$6)`, ["review", reviewId, body.verified ? "approve" : "reject", body.templateKey ?? null, body.note ?? null, actor.role]);
        await writeAuditLog({
            actorId: actor.id,
            actorRole: actor.role,
...
productsRouter.get("/moderation/questions/list", requireRole("viewer"), async (_req, res, next) => {
    try {
        const { rows } = await pool.query(`SELECT q.id, q.product_id, q.question, q.answer, q.author_name, q.author_email,
...
productsRouter.patch("/moderation/questions/:questionId", requireRole("manager"), async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`UPDATE product_questions
       SET answer = $2,
       ...
       RETURNING id, product_id, question, answer, author_name, author_email, created_at::text, answered_at::text,
                 ...
                 (SELECT sku FROM products WHERE products.id = product_questions.product_id) AS product_sku`, [questionId, answerValue]);
        if (rows.length === 0)
            return res.status(204).send();
...
productsRouter.post("/", requireRole("manager"), async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO products (name, sku, category, description, brand, material, size, voltage, finish, compatibility, warranty, safety_info, specs, manuals,
                             weight_kg, length_cm, width_cm, height_cm, backorderable, backorder_eta_days,
       ...
       RETURNING id, name, sku, category, description, brand, material, size, voltage, finish, compatibility, warranty, safety_info, specs, manuals,
        ...
        if (parsed.stock <= LOW_STOCK_THRESHOLD) {
            await pool.query(`INSERT INTO inventory_alerts (product_id, stock_level, threshold)
         VALUES ($1, $2, $3)`, [parsed.id, parsed.stock, LOW_STOCK_THRESHOLD]);
        }
        await writeAuditLog({
            actorId: actor.id,
            actorRole: actor.role,
...
productsRouter.put("/:id", requireRole("manager"), async (req, res, next) => {
    try {
        ...
        const previousStock = existing.rows[0]?.stock;
        const existingSpecs = (existing.rows[0]?.specs ?? null);
        const existingImageUrl = typeof existing.rows[0]?.image_url === "string" ? existing.rows[0].image_url : "";
        const inputImageUrls = body.imageUrls
        ...
        if (body.specs !== undefined || inputImageUrls !== null) {
            ...
            if (inputImageUrls !== null) {
                if (inputImageUrls.length > 0) {
                    ...
                    delete mergedSpecs.imageUrls;
                }
            }
        }
        const normalizedImageUrl = body.imageUrl?.trim() || (inputImageUrls && inputImageUrls[0]) || existingImageUrl || null;
       ...
       RETURNING id, name, sku, category, description, brand, material, size, voltage, finish, compatibility, warranty, safety_info, specs, manuals,
        ...
        if (typeof parsed.stock === "number") {
            if (parsed.stock <= LOW_STOCK_THRESHOLD) {
                const existingAlert = await pool.query("SELECT id FROM inventory_alerts WHERE product_id = $1 AND resolved_at IS NULL LIMIT 1", [parsed.id]);
                if (existingAlert.rowCount === 0) {
                    await pool.query(`INSERT INTO inventory_alerts (product_id, stock_level, threshold)
             VALUES ($1, $2, $3)`, [parsed.id, parsed.stock, LOW_STOCK_THRESHOLD]);
                }
                ...
                await pool.query("UPDATE inventory_alerts SET resolved_at = now() WHERE product_id = $1 AND resolved_at IS NULL", [parsed.id]);
            }
        }
        await writeAuditLog({
            actorId: actor.id,
