...
export const config = {
    ...
    paymentRedirectUrl: normalizeOrigin(process.env.PAYMENT_REDIRECT_URL ?? `${process.env.SITE_URL ?? "http://localhost:8081"}/checkout`),
    ...
    authCookieSecure: parseBoolean("AUTH_COOKIE_SECURE", (process.env.NODE_ENV ?? "development").toLowerCase() === "production"),
    ...
    exposeDebugTokens: parseBoolean("AUTH_EXPOSE_DEBUG_TOKENS", (process.env.NODE_ENV ?? "development").toLowerCase() !== "production"),
