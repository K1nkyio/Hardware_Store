...
export type UserRole = AdminRole | "customer" | "anonymous";
...
function normalizeTokenRole(input: unknown): UserRole {
  const value = typeof input === "string" ? input.trim().toLowerCase() : "";
  if (value === "super_admin" || value === "super-admin") return "super_admin";
...
function normalizeTokenDomain(input: unknown, role: UserRole): AuthDomain {
  const value = typeof input === "string" ? input.trim().toLowerCase() : "";
  if (value === "admin") return "admin";
  if (role === "viewer" || role === "manager" || role === "super_admin") return "admin";
  return "customer";
}
...
function parseAccessToken(req: Request): AccessTokenPayload | null {
  ...
  try {
    ...
    if (domain === "admin") {
      ...
      if (payload.type && !adminTypes.has(payload.type)) return null;
...
export function requireAdmin(options?: { allowMfaPending?: boolean }) {
  return (req: Request, res: Response, next: NextFunction) => {
    const actor = getActor(req);
