...
customerCareRouter.post("/inquiries", async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO customer_inquiries (first_name, last_name, email, order_number, message)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id::text, first_name, last_name, email, order_number, message, status,
                 moderation_note, moderated_by::text, moderated_at::text, created_at::text, updated_at::text`,
      [body.firstName, body.lastName, body.email, body.orderNumber ?? null, body.message]
    );
...
customerCareRouter.get("/inquiries", requireRole("viewer"), async (req, res, next) => {
  try {
    ...
    if (q) {
      ...
      whereParts.push(
        `(ci.first_name ILIKE $${idx} OR ci.last_name ILIKE $${idx} OR ci.email ILIKE $${idx} OR ci.order_number ILIKE $${idx} OR ci.message ILIKE $${idx})`
      );
    }
    ...
    const { rows } = await pool.query(
      `SELECT ci.id::text, ci.first_name, ci.last_name, ci.email, ci.order_number, ci.message, ci.status,
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE customer_inquiries
       ...
       RETURNING id::text, first_name, last_name, email, order_number, message, status,
                 moderation_note, moderated_by::text, moderated_at::text, created_at::text, updated_at::text`,
      [inquiryId, body.status, body.moderationNote ?? null, actor.id]
    );
