...
const mpesaVerifyCache = new Map<string, { checkedAt: number; payload: MpesaVerifyResponse }>();
type MpesaCallbackRecord = {
  checkedAt: number;
  resultCode: string;
  resultDesc: string;
  amountCents?: number;
...
const mpesaCallbackStore = new Map<string, MpesaCallbackRecord>();
...
async function readApiBody(response: Response): Promise<{ data: unknown; text: string }> {
...
function normalizeProviderError(err: unknown, fallback: string): string {
...
async function getDarajaAccessToken(): Promise<string> {
  try {
    ...
    const payload = data as { access_token?: string; errorMessage?: string; error_description?: string };
...
async function getPesapalToken(): Promise<string> {
  try {
    ...
    const { data, text } = await readApiBody(response);
    const payload = data as { token?: string; status?: string; message?: string };
...
paymentsRouter.post("/initialize", async (req, res, next) => {
  try {
    ...
    if (err && typeof err === "object" && "name" in err && String((err as { name?: string }).name) === "ZodError") {
...
paymentsRouter.post("/verify", async (req, res, next) => {
  try {
    ...
    const verified =
      (status === "completed" || status === "paid" || status === "successful") &&
...
function handleDarajaCallback(req: { body: unknown }, res: { status: (code: number) => { json: (payload: unknown) => unknown } }) {
