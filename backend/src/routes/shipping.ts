...
shippingRouter.post("/quote", async (req, res, next) => {
  try {
    ...
    if (body.items.length === 0) return res.status(400).json({ message: "No items provided" });
    ...
    const standardRate =
      billableWeight <= SMALL_ITEM_THRESHOLD_KG
        ? STANDARD_FLAT_RATE
        : Math.round(
            STANDARD_FLAT_RATE +
    ...
    const expressRate = Math.round(
      EXPRESS_BASE_RATE + citySurcharge + billableWeight * EXPRESS_SURCHARGE_PER_KG
    );
