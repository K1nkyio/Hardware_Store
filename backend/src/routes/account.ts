...
accountRouter.patch("/profile", requireAuth({ domain: "customer" }), async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query<{ full_name: string }>("SELECT full_name FROM users WHERE id = $1", [actor.id]);
...
accountRouter.get("/orders", requireAuth({ domain: "customer" }), async (req, res, next) => {
  try {
    const actor = getActor(req);
    const userRows = await pool.query<{ email: string }>("SELECT email FROM users WHERE id = $1", [actor.id]);
    if (userRows.rowCount === 0) return res.status(404).json({ message: "User not found" });
...
accountRouter.post("/payment-methods", requireAuth({ domain: "customer" }), async (req, res, next) => {
  ...
  try {
    ...
    if (!method) return res.status(500).json({ message: "Failed to create payment method" });
...
accountRouter.delete("/payment-methods/:id", requireAuth({ domain: "customer" }), async (req, res, next) => {
  try {
    ...
    if (!rowCount) return res.status(404).json({ message: "Payment method not found" });
    return res.status(204).send();
  } catch (err) {
    return next(err);
  }
