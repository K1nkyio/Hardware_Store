...
export function signToken(payload: Record<string, unknown>, secret: string, expiresInSeconds: number): string {
  const header = {
    alg: "HS256",
    typ: "JWT",
  };
...
export function verifyToken(token: string, secret: string): Record<string, unknown> | null {
  ...
  try {
    const payload = JSON.parse(decodeBase64Url(encodedBody).toString("utf-8")) as Record<string, unknown>;
    const now = Math.floor(Date.now() / 1000);
    const exp = Number(payload.exp ?? 0);
    if (!Number.isFinite(exp) || exp <= now) return null;
    const nbf = payload.nbf !== undefined ? Number(payload.nbf) : null;
    if (nbf !== null && (!Number.isFinite(nbf) || nbf > now)) return null;
    return payload;
  } catch {
    return null;
  }
...
export async function hashPassword(password: string, workFactor = 12): Promise<string> {
...
export async function verifyPassword(password: string, encodedHash: string): Promise<boolean> {
  ...
  if (scheme !== "scrypt" || !nRaw || !rRaw || !pRaw || !salt || !hashHex) return false;
  ...
  if (derived.byteLength !== expectedHash.byteLength) return false;
