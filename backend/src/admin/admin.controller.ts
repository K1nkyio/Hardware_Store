...
function statusForError(err: unknown): number {
  ...
  if (message.includes("mfa")) return 401;
...
export async function mfaSetup(req: Request, res: Response, next: (err?: unknown) => void) {
  try {
    const actor = getActor(req);
    if (!actor.mfaPending) return res.status(400).json({ message: "MFA setup is not pending for this session" });
...
export async function mfaActivate(req: Request, res: Response, next: (err?: unknown) => void) {
  try {
    const actor = getActor(req);
    if (!actor.mfaPending) return res.status(400).json({ message: "MFA setup is not pending for this session" });
...
export async function mfaRegenerateRecoveryCodes(req: Request, res: Response, next: (err?: unknown) => void) {
  try {
    const actor = getActor(req);
...
export async function mfaReset(req: Request, res: Response, next: (err?: unknown) => void) {
  try {
    const actor = getActor(req);
